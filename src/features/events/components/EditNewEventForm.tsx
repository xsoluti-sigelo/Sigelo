...
import { updateEventFormAction } from '../api/form-actions'
...
import { DocumentUploadZone, DocumentListView } from '@/features/documents/components'
...
import { downloadDocument } from '@/features/documents/actions/download-document'
import { deleteEventAttachment } from '@/features/documents/actions/delete-document'
import { EventType } from '@/features/events/config/event-types'
import type { EditEventFormProps } from '../types'
import { logger } from '@/shared/lib/logger'

type SectionKey = 'eventData' | 'location' | 'schedule' | 'services' | 'people' | 'documents'

export type EditNewEventFormProps = EditEventFormProps & {
  variant?: 'manual' | 'auto'
}

export function EditNewEventForm({
  eventId,
  initialData,
  variant = 'manual',
}: EditNewEventFormProps) {
  ...
  const { formState, setters, actions } = useEditEventForm({ eventId, initialData })

  const { formAction, isPending } = useServerActionWithRefresh(updateEventFormAction, {
    onSuccess: () => {
      actions.handleSaveSuccess()
    },
    onError: (error) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let extractedStartDate = formState.startDate
    let extractedStartTime = formState.startTime
    let extractedEndDate = formState.endDate
    let extractedEndTime = formState.endTime

    if (mobilizationDatetimeValue) {
      const [mobDate, mobTime] = mobilizationDatetimeValue.split('T')
      if (mobDate) extractedStartDate = mobDate
      if (mobTime) extractedStartTime = mobTime
    }

    if (demobilizationDatetimeValue) {
      const [demobDate, demobTime] = demobilizationDatetimeValue.split('T')
      if (demobDate) extractedEndDate = demobDate
      if (demobTime) extractedEndTime = demobTime
    }
      ...
      <div className="grid gap-6 lg:grid-cols-2 items-start">
        ...
        >
          <div className="space-y-4">
            ...
            {isAuto && (
              <div className="flex flex-wrap gap-6 border-t border-dashed border-gray-200 pt-4">
                <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                  <input
                    type="checkbox"
                    checked={formState.isNightEvent}
                    onChange={(e) => setters.setIsNightEvent(e.target.checked)}
                ...
                <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                  <input
                    type="checkbox"
                    checked={formState.isIntermittent}
                    onChange={(e) => setters.setIsIntermittent(e.target.checked)}
                    className="h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500"
                  />
                  Evento intermitente
                </label>
        ...
        >
          <div className="space-y-6">
            {initialData.attachments && initialData.attachments.length > 0 && (
              <DocumentListView
                documents={initialData.attachments.map((attachment) => ({
                  id: attachment.id,
                ...
                onDownload={(storagePath, fileName, id) =>
                  handleDownload(
                    ...
                    (params) => downloadDocument(params.storagePath, params.fileName),
                    id,
                  )
                }
                onDelete={async (id, storagePath) => {
                  await deleteEventAttachment(id, storagePath, eventId)
                  window.location.reload()
                }}
                isDownloading={isDownloading}
                emptyTitle="Nenhum anexo encontrado"
                emptyDescription="Anexos do evento serão exibidos aqui"
              />
            )}
            ...
            <DocumentUploadZone
              ...
              existingFilesCount={initialData.attachments?.length ?? 0}
              onFilesChange={(files) => {
